...
export default function EnergijaVerification({ navigation }) {
  ...
  const submitImage = () => {
    if (!receiptImage) {
      Alert.alert("Upozorenje", "Molimo vas da prvo slikate račun za struju!");
      return;
    }
    Alert.alert("Uspješno", "Račun za struju je uspješno priložen!");
  };
  ...
  return (
    <ImageBackground source={require("../../assets/ion.jpg")} style={styles.background}>
      <View style={styles.container}>
        ...
        <Text style={styles.title}>Priloži račun za struju</Text>
        ...
        <TouchableOpacity style={styles.submitButton} onPress={submitImage}>
          <Text style={styles.buttonText}>Priloži račun</Text>
        </TouchableOpacity>
      </View>
    </ImageBackground>
  );
